...
**context 滿的時候** — 現實中，大部分對話不會用 `/exit` 正式結束。通常是 context 滿了、自動壓縮、繼續講。`PreCompact` hook 就在這個時間點觸發，壓縮前先存一份快照 — 跟 SessionEnd 做一樣的事（摘要、踩坑偵測、備份）。這才是真正的安全網。
...
- 大部分對話不會用 `/exit` 結束 — PreCompact 補上 SessionEnd 漏接的
