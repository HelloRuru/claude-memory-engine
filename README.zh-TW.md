...
| 功能 | 說明 |
| :--- | :--- |
| 迴圈 | **上課抄筆記**：自動記重點 → 串連同專案 → 找規律（前 3 步自動）。**期末總複習**：回顧 → 精煉 → 再學 → 瘦身 → 收尾（後 5 步，`/反思` 觸發） |
| 中繼 | 每 20 則訊息自動存一份中繼紀錄，附帶 mini 分析（在做什麼、碰了哪個專案） |
| 提醒 | 開新對話時檢查上次跑 `/反思` 是什麼時候，超過 7 天就提醒 |
| 踩坑 | 同一種錯跨天出現 3 次以上？開對話時主動建議寫進長期規則 |
| 四問 | `/反思` 步驟 5 改用四問判斷：要長嗎？→ 能濃縮？→ 已有規則涵蓋？→ 清除是最後手段 |
| 修正 | 修好從 v1.0 就壞掉的 transcript 解析、加 IDE 雜訊過濾、踩坑門檻調高到 5 |
...
| 功能 | 說明 |
| :--- | :--- |
| 14 個指令 | 完整指令集：日常操作、健檢、備份、學習、待辦追蹤 |
| 雙語 | 每個指令都有英文 + 中文版（共 28 個檔案） |
| 兩階段健檢 | `/健檢` 日常掃描 + `/大健檢` 每週稽核 |
| 記憶快捷 | `/存記憶` `/讀取` `/同步`，從日常工作流整理出來的 |
| `/學習` | 踩坑自動學習的獨立指令 |
| `/待辦` | 跨專案待辦追蹤 |
| `/想起來` | 記憶不見時的緊急還原 |
...
| 功能 | 說明 |
| :--- | :--- |
| 自動偵測 | Smart Context 不用再手動設定，自動掃描所有專案的記憶目錄 |
| 中文糾正 | 踩坑偵測認得 13 個中文詞 |
| 搜尋記憶 | `/搜尋記憶` 用關鍵字搜尋所有記憶檔案 |
| 含解法 | 踩坑紀錄自動從對話中抓出解法 |
| 摘要改版 | 包含「做了什麼」主題摘錄 |
| 自動週報 | 超過 7 天的 session 自動合併成週報 |
