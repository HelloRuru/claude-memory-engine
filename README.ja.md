...
**context が満杯になった時** — 現実では、ほとんどの会話は `/exit` で正式に終了しない。context が満杯になって自動圧縮され、そのまま会話が続く。`PreCompact` hook はこのタイミングで発火し、圧縮前にスナップショットを保存する — SessionEnd と同じ処理（要約、落とし穴検出、バックアップ）。これが本当のセーフティネット。
...
- ほとんどの会話は `/exit` で終了しない — PreCompact が SessionEnd の漏れを補完
