...
| 機能 | 説明 |
| :--- | :--- |
| ループ | **授業ノート**：要点を自動記録 → プロジェクト別に連結 → パターン発見（前半 3 ステップ、自動）。**期末総復習**：見直し → 精錬 → 再学習 → スリム化 → まとめ（後半 5 ステップ、`/reflect`） |
| 中間記録 | 20 メッセージごとに自動保存、ミニ分析付き（何をしていたか、どのプロジェクトか） |
| リマインド | 前回 `/reflect` から 7 日以上経過していたら通知 |
| 踏み抜き | 同じミスが 3 日以上出現 → 永久ルールへの書き込みを提案 |
| 四問判断 | `/reflect` ステップ 5：成長すべき? → 凝縮? → 既存ルールでカバー済み? → 削除は最後の手段 |
| バグ修正 | transcript パーサー修正（v1.0 から壊れていた）、IDE ノイズ除去、閾値を 5 に引き上げ |
...
| 機能 | 説明 |
| :--- | :--- |
| 14 コマンド | 日常操作、ヘルスチェック、バックアップ、学習、タスク追跡 |
| 二言語 | 全コマンドに英語 + 中国語版（28 ファイル） |
| ヘルス | `/check` 日次 + `/full-check` 週次の二段階チェック |
| ショートカット | `/save` `/reload` `/sync` メモリ操作の自然言語コマンド |
...
| 機能 | 説明 |
| :--- | :--- |
| 自動検出 | Smart Context の手動設定が不要に、全プロジェクトを自動スキャン |
| 中国語対応 | 踏み抜き検出が 13 の中国語フレーズに対応 |
| 検索 | `/memory-search` でキーワード横断検索 |
| 解決策記録 | エラーだけでなく成功した修正も自動抽出 |
| 要約改善 | 「何をしたか」のトピック抽出を含む形式に |
...
| 復旧 | `/recover` で GitHub バックアップから復元 |
| 二言語 | 全コマンドに英語 + 中国語版（28 ファイル） |
| バックアップ | `/backup` `/sync` で GitHub と双方向同期 |
...
| 項目 | 場所 |
| :--- | :--- |
| 対応表 | Smart Context v1.1 で自動検出（通常設定不要）、`session-start.js` で調整 |
